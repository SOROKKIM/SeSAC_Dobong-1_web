...
const PostList = ({ posts }) => {
  return (
    <>
      <h2 style={titleStyle}>📨 Post List</h2>
          ...
          <ul>
            {posts.map((post, index) => (
              <li key={post.id} style={postStyle}>
                <h3 style={{ ...postNumberStyle, display: "inline" }}>{`NO. ${
                  index + 1
                }`}</h3>
                <h3
                  style={{ ...postTitleStyle, display: "inline" }}
                >{` - ${post.title}`}</h3>
                <p>{post.body}</p>
              </li>
            ))}
...
export default function PracticeLifeCycleFunc() {
  const [posts, setPosts] = useState([]);

  useEffect(() => {
    // axios를 사용하여 JSONPlaceholder에서 데이터를 가져오기
